"""
Script 2 — Napari Region Selection: Dorsal White Matter
=========================================================
Purpose:
  Manually select the dorsal white matter region per sample using Napari.
  Run this script once per sample by changing SAMPLE and CONDITION below.

Workflow:
  1. Set SAMPLE and CONDITION at the top
  2. Run the script — Napari opens with the hexbin tissue map
  3. Draw a polygon around the dorsal white matter using the Shapes layer
...
# Headmap where this script is stored
BASE_DIR = os.getcwd()

INPUT_PATH = os.path.join(BASE_DIR, "Hexbin_data")

if CONDITION == 'ALS':
    OUTPUT_PATH = os.path.join(BASE_DIR, "output", "Region_clusters", "Disease_hexpaint", "Dorsal")
    FILE_PREFIX = f"ALS_{SAMPLE}"
else:
    OUTPUT_PATH = os.path.join(BASE_DIR, "output", "Region_clusters", "Healthy_hexpaint", "Dorsal")
...
# ════════════════════════════════════════════════════════════════════════════════
print(f"\n{'='*55}")
print(f"  Dorsal WM Selection — {SAMPLE} ({CONDITION})")
print(f"{'='*55}")
...
print(f"\n  Opening Napari — draw a polygon around the DORSAL white matter")
print(f"  (posterior/sensory tract, opposite side from the ventral horn)")
print(f"  then close the window to save the selection.")

viewer = napari.Viewer(title=f"Dorsal WM Selection — {SAMPLE} ({CONDITION})")
viewer.add_points(
...
if shapes_layer is None or len(shapes_layer.data) == 0:
    ...
    output_file    = os.path.join(OUTPUT_PATH, f"{FILE_PREFIX}_selection_Dorsal.csv.gz")
    ...
    fig.suptitle(f'Dorsal WM Selection Control Plot — {SAMPLE} ({CONDITION})',
    ...
    axes[0].scatter(selection['hexbin_i'], selection['hexbin_j'],
                    s=1, c='#588157', alpha=0.9, label='Dorsal WM selection')
    axes[0].set_aspect('equal')
    axes[0].set_title('Dorsal WM selection on full tissue')
    ...
    axes[1].bar(cluster_counts.index.astype(str), cluster_counts.values,
                color='#588157', alpha=0.8)
    axes[1].set_title('Cluster composition of Dorsal WM selection')
    axes[1].set_xlabel('Cluster')
    axes[1].set_ylabel('Number of hexbins')
    axes[1].grid(axis='y', alpha=0.3, linestyle='--')

    plt.tight_layout()
    control_plot = os.path.join(OUTPUT_PATH, f\"control_{FILE_PREFIX}_Dorsal.png\")
    plt.savefig(control_plot, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"  Control plot saved: control_{FILE_PREFIX}_Dorsal.png")

    print(f"\n  Done! Summary:")
    print(f"    Sample:     {SAMPLE} ({CONDITION})")
    print(f"    Region:     Dorsal white matter")
